    ...
    <dialog ref={dialog} className="task-dialog" aria-labelledby="dialog-title" onCancel={(event) => {
      ...
      <div className="dialog-header">
        <div>
          ...
          <h2 id="dialog-title">{taskId === null ? 'Новая задача' : editing ? 'Редактирование задачи' : 'Детали задачи'}</h2>
        </div>
      ...
      {error && <div className="error-message" role="alert">
        ...
        {taskId !== null && (!task || error.status === 409) && <button className="text-button" onClick={() => { setEditing(false); setReload(value => value + 1); }}>Обновить задачу</button>}
      </div>}

      {loading ? <div className="dialog-loading" role="status"><span className="spinner" />Загружаем задачу…</div> : editing ? (
        <form onSubmit={save}>
